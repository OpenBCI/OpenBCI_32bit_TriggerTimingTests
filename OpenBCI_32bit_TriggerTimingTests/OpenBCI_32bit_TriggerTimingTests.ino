 ...
 *
 *      This code will read an analog sensor, and put the ADC result into the AUX data
 *      Assumes an analog sensor connected to pin D13 (A7)
 *
...
//------------------------------------------------------------------------------
//  << SENSOR STUFF >>
int analogSensor = A7; // Analog sensor output tied to pin A7 (D13)
int sensorValue;       // holds the sensor value
//------------------------------------------------------------------------------
...
void loop() {
  ...
  if(is_running){
      ...
      OBCI.auxData[2] = sensorValue;  // auxData[] is array of shorts (16bit values)
      addAuxToSD = true;  // add the aux data to the SD card, if you like
